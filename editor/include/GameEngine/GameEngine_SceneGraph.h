...
namespace GameEngine
{
	...
	/**
	 *Creates a game entity and returns its world id from the given attachment.
	 */
	...
	*/
	...
	/**
	 * set the stereoscopy mode (0 = disabled, 1 = side-bye-side, 2 = quad-buffered)
	*/
	...
	*/
	...
	/**
	 * set the stereoscopy method (0 = toed-in stereo (uses strabismus parameter), 1 = asymmetric viewing frustum (uses focal length parameter))
	*/
	SCENEGRAPHAPI void setStereoscopyMethod(unsigned int method);
	/**
	 * set stereoscopy parameters
	*/
	SCENEGRAPHAPI void setStereoscopyParams(float eyeOffset, float strabismus, float focalLength);
	/**
	 * get the stereoscopy parameters
	*/
	SCENEGRAPHAPI void getStereoscopyParams(float& eyeOffset, float& strabismus, float &focalLength);
	/**
	 * set stereoscopy pipeline
	*/
	SCENEGRAPHAPI void setStereoscopyPipeline(const char* pipeline);
	/**
	 * set stereoscopy pipeline
	*/
	SCENEGRAPHAPI void setStereoscopyOverlayMaterial(const char* material);
