...
def object_build_function(node, member, localname):
    ...
    func = build_function(getattr(member, '__name__', None) or localname, args,
                          defaults, member.__code__.co_flags, member.__doc__)
    ...
    def object_build(self, node, obj):
        ...
        for name in dir(obj):
            ...
            if isfunction(member):
                ...
                if member.__code__.co_filename != getattr(self._module, '__file__', None):
            ...
            elif "__get__" in dir(member):#ismethoddescriptor(member):
                assert isinstance(member, object)
                object_build_methoddescriptor(node, member, name)
            elif "__get__" in dir(member) and "__set__" in dir(member):#isdatadescriptor(member):
                assert isinstance(member, object)
...
def astng_boot_strapping():
    ...
    for cls, node_cls in list(CONST_CLS.items()):
        if cls is type(None):
