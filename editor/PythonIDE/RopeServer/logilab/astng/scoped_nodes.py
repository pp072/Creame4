...
from .node_classes import Assign, AssAttr, CallFunc, For, Discard, Name, Const
...
class Class(Statement, LocalsDictNodeNG, FilterStmtsMixin):
    ...
    def set_metaclass(self):
        """Parse and set variables from metaclass
        """
        self.is_meta_end = True
        metas = []
        for meta in unpack_infer(self.metaclass):
            for base in meta.bases:
                for b in unpack_infer(base):
                    if b not in metas:
                        self.set_meta_object(b)
                        metas.append(b)
            if meta not in metas:
                self.set_meta_object(meta)
                metas.append(meta)
        
        
    def set_meta_object(self, meta):
        def get_for_iteration(generator):
            for it in generator:
                return [item for item in it.get_children()]
                
        def assign(item):
            if item.targets[0].expr.name == mcls:
                self.set_local(item.targets[0].attrname, item)
                
        def func(item):
            if not( isinstance(item.value.func, Name) \
                and item.value.func.name == "setattr" \
                and item.value.func.parent.args[0].name == mcls):
                return
                
            iname = item.value.func.parent.args[1]
            if isinstance(iname, Name) and iname.name in vars:
                self.set_local(vars[iname.name], item.value.func.parent.args[2])
                
        inits = []
        inits.append(meta.getattr("__new__")[0])
        #inits.append(meta.getattr("__init__")[0])
        ...
        for init in inits:
            if len(init.argnames()) == 0:
                continue
            mcls = init.argnames()[0]
            for child in init.get_children():
                if isinstance(child, Assign) and isinstance(child.targets[0], AssAttr):
                    assign(child)
                elif isinstance(child, For):
                    try:
                        iter = get_for_iteration(self.igetattr(child.iter.attrname))
                    except AttributeError:
                        for aa in child.iter.func.get_children():
                            iter = self.igetattr(aa.attrname)
                    for it in iter:
                        vars = {
                            child.target.name : it
                        }
                        for for_child in child.get_children():
                            if isinstance(for_child, Assign) and isinstance(for_child.targets[0], AssAttr):
                                assign(for_child)
                            if isinstance(for_child, Assign):
                                for tar in for_child.targets:
                                    if tar.__class__.__name__ == "AssName":
                                        for vv in tar.assigned_stmts():
                                            vars[tar.name] = vv.value
                                    if tar.__class__.__name__ == "Tuple":
                                        tupl = None
                                        
                                        for vv in for_child.assigned_stmts(tar):
                                            if vv.name in vars:
                                                tupl = vars[vv.name]
                                        ...
                                        for count, tup in enumerate(tar.get_children()):
                                            if isinstance(tupl.elts[count], Const):
                                                vars[tup.name] = tupl.elts[count].value
                                            elif hasattr(tupl.elts[count], "name"):
                                                vars[tup.name] = tupl.elts[count].name
                                                
                            if isinstance(for_child, Discard) and isinstance(for_child.value, CallFunc):
                                func(for_child)
    ...
    def getattr(self, name, context=None):
        ...
        """
        if self.metaclass and not self.is_meta_end:
            self.set_metaclass()
