...
void NodeWidget::transformChanged( const float tx, const float ty, const float tz, const float rx, const float ry, const float rz, const float sx, const float sy, const float sz )
{
	if( !m_currentNode || !m_currentNode->property("Position").isValid() ) return;
	...
	m_currentNode->setProperty("Position", QVariant::fromValue(QVec3f(m_tx, m_ty, m_tz)));
	m_currentNode->setProperty("Scaling", QVariant::fromValue(QVec3f(m_sx, m_sy, m_sz)));
}
