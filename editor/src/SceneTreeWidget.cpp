...
SceneTreeWidget::SceneTreeWidget(QWidget* parent /*= 0*/, Qt::WFlags flags /*= 0*/) : QWidget(parent, flags)
{
	...
	connect(m_sceneTreeView, SIGNAL(currentNodeChanged(QXmlTreeNode*)), this, SLOT(sendHordeID(QXmlTreeNode*)));
...
void SceneTreeWidget::sendHordeID( QXmlTreeNode* node)
{
	if(!node)
		return;
	unsigned int ID = node->property("ID").toInt();
	emit signal_selectionChanged(ID);
}
