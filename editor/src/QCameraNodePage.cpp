...
//m_init(init)
{
	...
	registerField("pipeline", m_pipeline, "currentText", SIGNAL(currentIndexChanged ( int )));
	//connect( m_pipeline, SIGNAL(currentIndexChanged ( QString)), this, SIGNAL( completeChanged() ) );
...
void QCameraNodePage::initializePage()
{
	...
	QDir dir = QDir::currentPath()+"/pipelines";
	...
	for (int i = 0; i < list.size(); ++i) {
		...
		m_pipeline->addItem(QString("pipelines/")+fileInfo.fileName());
	}
