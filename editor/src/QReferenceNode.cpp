	...
	QSceneNode(xmlNode, row, model, parentNode), m_treeModel(0)
{
	...
	{
		attachment.save(stream, 4);
		//SceneTreeModel* model = static_cast<SceneTreeModel*>(m_model);
		h3dSetNodeParamStr(m_hordeID, H3DNodeParams::AttachmentStr, qPrintable(str));
	}
}
...
{		
	...
	{		
		editor::instance()->getLog_window()->updateLog(new QListWidgetItem(tr("Error when opening file %1!\n%2").arg(m_fileName).arg(sceneGraphFile.errorString()), 0, 2));
		m_hordeID = 0;
		return;
	}
	QString errorMsg;
	...
	{
		editor::instance()->getLog_window()->updateLog(new QListWidgetItem(tr("Error in XML file %1, line %2 column %3:\n%4").arg(m_fileName).arg(line).arg(column).arg(errorMsg), 0, 2));		
		m_hordeID = 0;
		return;
	}
	sceneGraphFile.close();
	...
	m_treeModel = new SceneTreeModel(editor::instance()->pluginManager(), m_sceneGraphFile.documentElement(), this);
